...
# 1. 缺陷能级分流与工程红线 (Defect Triage & Redlines)

在处理任何 Bug 或异常前，**必须首先执行缺陷能级分流 (Triage)，严禁不分青红皂白过度工程化**：

### 🚦 缺陷能级分流矩阵 (Triage Matrix)

| 能级分类 | 典型场景 | 执行流转协议 | 阻断要求 |
| :--- | :--- | :--- | :--- |
| 🟢 **L0 轻量自明性瑕疵 (Fast-Track 极速通道)** | Markdown 链接/路径兼容性修正、Typo 拼写、非逻辑样式微调、注释增补、单点静态配置调整、用户已明确指定改动位置的直白微调 | **免除写锁与长篇 RCA 报告**，直接调用文件修改工具进行最小原子修复，提供 diff 或执行结果，一轮极速闭环 | ❌ 不阻断，坚决杜绝小题大做 |
| 🟡 **L1 局部业务与逻辑缺陷 (Standard 流水线)** | 单模块内部逻辑不闭环、边界空值判空缺失、参数反序列化失败、单元测试断言失败 | 给出轻量级 3 行根因说明（Root Cause -> 方案 -> 影响行数），快速对齐后修复 | ⚠️ 简报对齐后下刀 |
| 🔴 **L2 系统高危与并发缺陷 (Heavy-Weight 严苛门禁)** | Panic 崩溃、高并发死锁/竞态、内存泄露、数据库事务/锁冲突、状态机异常流转、跨微服务契约断裂 | **强制执行强只读写锁**，严格走 SOP 五步闭环，必须出示结构化 RCA 报告经用户明确审阅同意后方可解封修改 | 🚨 强阻断，未经确认严禁动代码 |

---

## 核心工程红线

在进入代码排查与修改前，必须时刻遵守以下铁律：

### 🚨 铁律一：复杂缺陷严禁先斩后奏盲改代码 (No Blind Patching for Complex Bugs)
- 仅对 **L1/L2 级缺陷** 生效：**严禁直接修改代码并丢给用户一个改动后的版本**。
- **强制执行机制**：必须先指出根本原因（Root Cause）、复现逻辑以及潜在影响，提供明确的修复方案。**待用户审阅确认后，方可进行代码修复**。
- **L0 级极速豁免**：针对 L0 级轻量自明性瑕疵，**坚决禁止走冗长的审阅流拖慢心流，直接敏捷修复并回显变更事实**。
