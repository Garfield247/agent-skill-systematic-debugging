...
description: >-
  ...
  坚决杜绝静默添加重试或默认值兜底、最小破坏性原子修复与单测/日志验证标准。
---
...
AI 在面对 Bug 时最容易犯的错误是：**“跳过思考直接改代码”**，以及**“好心办坏事——静默添加重试或兜底默认值掩盖真问题”**。本技能旨在将顶尖架构师的诊断智慧工程化，确保每一次排错都做到：**机理清楚、因果明确、方案受控、无副作用**。
...
flowchart TD
    ...
    D --> E{"用户审阅确认?"}
    E -- 调整/否定 --> C
    E -- 确认同意 --> F["5. 实施原子修复与验证<br/>(Fix & Verification)"]
```
...
## 阶段 5：实施原子修复与回归验证 (Fix & Verification)
- 经用户确认后，采用最精确的代码修改；
- **自愈与回归验证**：
  - 必须提供验证步骤（如具体的 curl 请求样例、单测断言、日志验证规则）；
  - 补充针对该 Bug 的单元测试/边界测试用例，防止历史缺陷在未来重构中回潮。
...
> 💡 请审阅以上根因剖析与修复思路。如无疑问，请回复确认，我将立即实施原子修复并附带验证用例。
```
...
- [ ] 是否提供了确切的复测验证手段（单测、curl、日志证据）？

---

# 6. 高级排障门禁：红绿灯测试先行与事后五步复盘 (Advanced Diagnostic Guardrails)

吸纳顶级自动化排障团队（SWE-bench / Devin）的最佳工程门禁：
